...
export default function WebinarEvents() {
  ...
  const generateCircular = async (month) => {
    try {
      ...
      // Prepare table data for preview with batch fetched from members table
      const tableData = await Promise.all(monthWebinars.map(async (webinar) => {
        ...
        // Fetch batch from members table using speaker's email
        let batchInfo = 'TBD';
        if (webinar.speaker?.email) {
          try {
            const response = await fetch(`http://localhost:5000/api/member-by-email?email=${encodeURIComponent(webinar.speaker.email)}`);
            if (response.ok) {
              const memberData = await response.json();
              if (memberData.found) {
                batchInfo = memberData.batch || 'TBD';
              }
            }
          } catch (error) {
        ...
        const speakerWithDetails = `${speakerInfo}\n(Batch ${batchInfo} - ${departmentInfo})`;
        return {
          branch: departmentInfo,
          date: formattedDate,
...
function signatureCell(title) {
  ...
  const WebinarCard = ({ webinar }) => {
    ...
    return (
      <div className="form-card p-6 flex flex-col h-full webinar-card relative">
        <button
          onClick={() => navigate(`/webinar-details-upload/${webinar._id}`)}
          className="absolute top-4 right-4 bg-purple-600 text-white p-2 rounded-lg hover:bg-purple-700 transition-colors"
          title="Upload Webinar Details"
        >
          <FiUpload size={27} />
        </button>
        ...
        <p className="text-lg text-gray-700 mb-4">
          ...
          {webinar.meetingLink ? (
            ...
            <span className="text-gray-500 font-medium">{webinar.venue || 'Offline Details'}</span>
          )}
        </p>

        <div className="mt-auto space-y-2">
          <button
            onClick={() => !isRegistered && !isDeadlinePassed && setSelectedWebinar(webinar)}
            className={`submit-btn webinar-card-btn mb-2 ${(isRegistered || isDeadlinePassed) ? 'opacity-50 cursor-not-allowed bg-gray-400' : ''}`}
            disabled={isRegistered || isDeadlinePassed}
          >
            {isRegistered ? 'Registered' : isDeadlinePassed ? 'Deadline Passed' : 'Register'}
          </button>

          <button
            onClick={() => navigate(`/student-feedback?webinarId=${webinar._id}&topic=${encodeURIComponent(webinar.title)}&speaker=${encodeURIComponent(webinar.speaker.name)}`)}
      ...
      {showCircularPreview && circularData && (
        <div className="fixed inset-0 bg-white/85 bg-opacity-30 flex items-center justify-center p-5 z-50">
          <div className="bg-gradient-to-br from-purple-50/70 via-pink-50/70 to-blue-50/70
            ...
            <div className="flex justify-center">
              <button onClick={downloadCircular} className="submit-btn">
                Download Circular
              </button>
            </div>
