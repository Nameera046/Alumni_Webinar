...
router.get('/topic-approvals', async (req, res) => {
  try {
    let topicApprovals = await req.app.locals.TopicApproval.find({});

    // If no topic approvals exist, generate them from student requests
    if (topicApprovals.length === 0) {
      console.log('No topic approvals found, generating from student requests...');
      ...
      if (studentRequests.length > 0) {
        ...
        if (newTopicApprovals.length > 0) {
          ...
          console.log(`Generated ${newTopicApprovals.length} topic approvals`);
        }
      }
    }
...
router.get('/student-requests/:domain/:topic', async (req, res) => {
  try {
    ...
    // Find student requests that match the domain and have similar topics
    const studentRequests = await req.app.locals.StudentRequestForm.find({ domain });
