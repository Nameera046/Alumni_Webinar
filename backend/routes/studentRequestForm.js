...
router.post('/submit-student-request', async (req, res) => {
  try {
    const { email, domain, topic, reason } = req.body;
    ...
    // Update topic approval table by re-grouping all topics for this domain
    const allRequestsForDomain = await req.app.locals.StudentRequestForm.find({ domain });
    const topicsForDomain = allRequestsForDomain.map(request => request.topic);

    const groupedTopics = groupTopics(topicsForDomain);

    // Clear existing topic approvals for this domain
    await req.app.locals.TopicApproval.deleteMany({ domain });
